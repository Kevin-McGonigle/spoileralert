function createOverlays(elements, indexes){
    ...
    {
        elements[i].setAttribute("aria-hidden", "true");  // Hides text from screen readers
        let e = elements[i];
        let h = $(e).innerHeight();
        let w = $(e).innerWidth();
        ...
        overlay.alt = "SpoilerAlert overlay"
        ...
        reveal.alt = "Reveal Button, click to reveal the text";
        ...
        if (h >= 22 && w >= 40) {
            if (h < 40) {
                ...
                if (w < 80){
                    ...
                    logo.style.width = (Math.min.apply(Math, [h, w/2]) - 16) + "px";
                    reveal.style.width = (Math.min.apply(Math,[h, w/2]) - 16) + "px";
                    if (w >= 180){
                        overlay.innerText = "Spoiler Alert";
                    }
                }
    ...
    if (elements.length > 0){
        $.ajax({
            ...
            success: function (indexes) {
                ...
                spoiler_count += indexes.length;
                chrome.runtime.sendMessage({message : "spoilerCount " + spoiler_count})
            },
            error: function () {
...
function subscriber(mutations){
    for (let mutation of mutations){
        for (let node of mutation.addedNodes){
            if (!($(node).hasClass("sa") || $(node.parentElement).hasClass("sa"))){
                send($.map($(node).find('p, h1, h2, h3, h4, h5, h6, td, th, li'), matchText));
            }
        }
    }
}
...
function matchText(element) {
    let noSpoilerPattern = /No Spoilers/i
    if (noSpoilerPattern.test(element.innerText))
        return null
    let SpoilerPattern = /Spoilers/i
    if (SpoilerPattern.test(element.innerText))
        return element
...
$(document).ready(function () {
    chrome.runtime.sendMessage({message: "validSite"});
    ...
    send($.map($('p, h1, h2, h3, h4, h5, h6, td, th, li'), matchText));
